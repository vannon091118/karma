...
def _load_manifest() -> Dict[str, Any]:
    ...
    from karma.skills.registry import _load_manifest as _registry_load_manifest
...
def prompt_quick(project: str, skill: str, platform: str):
    ...
    from karma.skills.registry import discover_skills, _map_skills_to_domains
...
@click.option("--force", is_flag=True, help="Overwrite existing .karma/ if present")
def init(project: Optional[str], force: bool):
    """Initialize a project: detect context, sync memory, write onboarding files.

    Creates .karma/ with index.md (domain TOC), inventory.md (skill
    ...
    if dot.exists() and not force:
        console.print(f"[yellow].karma/ already exists in {proj_dir} — use --force to overwrite[/yellow]")
        return
    ...
    from karma.skills.registry import (
discover_skills,  _map_skills_to_domains,    )
    ...
    snippet = (
        ...
        "This project uses `karma` as a persistent context layer. Available commands:\n"
        f"- `karma status --project {proj}` — show sync state\n"
        "- `karma memory get <domain> [key]` — read project memory\n"
        "- `karma skill list` — list available skills\n"
        f"- `karma dispatch '<request>' --project {proj}` — auto-select skills & build delegate tasks\n"
    )
    claude = proj_dir / "CLAUDE.md"
    existing = claude.read_text(encoding="utf-8") if claude.exists() else ""
    if "karma" not in existing:
...
def onboard(project: Optional[str]):
    ...
    from karma.skills.registry import discover_skills
    skills = discover_skills()
    manifest = _load_manifest()
    domains = manifest.get("domains", {})
    ...
    console.print(f"Next steps: `karma status --project {name}` to verify.")
...
def status(project: Optional[str]):
    ...
    if loaded:
        ...
        console.print("║    (none loaded — use 'karma skill load <skill|group>')     ║")
...
def skill_unload(target: str):
    ...
    from karma.skills.registry import discover_skills, cmd_unload, cmd_ungroup
    skills = discover_skills()
    manifest = _load_manifest()
    groups = manifest.get("skill_groups", {})
...
def skill_load_all():
    """Load all skills."""
    from karma.skills.registry import cmd_load_all
    cmd_load_all()
...
def skill_unload_all():
    """Unload all skills."""
    from karma.skills.registry import cmd_unload_all
    cmd_unload_all()
...
def skill_context(skill_name: str):
    ...
    from karma.skills.registry import cmd_context
...
def dispatch(request: str, project: Optional[str], platform: str):
    ...
    from karma.skills.registry import discover_skills, _map_skills_to_domains
    from karma.core.context_optimizer import assemble_context
    from karma.prompt_engine import generate_prompt
...
def export(platform: str, group: Optional[str], force: bool, dry_run: bool):
    ...
    from karma.runtime.migrate_skills import main as migrate_main
