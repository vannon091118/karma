...
from llm_middleware.runtime import memory_core, context_optimizer
from llm_middleware.runtime import skill_registry as _skill_registry
...
def generate_full_prompt(project: str, skill_name: str,
    ...
    # Load skill content
    skills = _skill_registry.discover_skills()
    skill_content = ""
...
def cmd_generate(argv: List[str]) -> None:
    ...
    if not domains:
        skills = _skill_registry.discover_skills()
        manifest = _skill_registry._load_manifest()
        domain_map = _skill_registry._map_skills_to_domains(skills, manifest)
